        ...
        <div class=" card card-warning card-outline ">
            ...
            <!--                <h3 class="card-title">{{$title}} List</h3>-->
                ...
                <div class="card-tools pr-5">
                    <a href="{{route('itemType.pdf')}}">View</a>
                </div>
            </div>
                    ...
                    @foreach($items as $g)
                        <tr>
                            ...
                            <td id="table-show_name" class="mailbox-date">
                                ...
                                <form id="table-show_name-form{{$g->id}}" action="{{route($link.'.update',[$link=>$g->id])}}" method="post" style="display: none">
                                    @csrf
                                    @method('PATCH')
                            ...
                            <td class="mailbox-attachment">
                                ...
                                <form id="status-{{$g->id}}" action="{{ route($link.'.update',[$link=>$g->id]) }}" method="POST" style="display: none;">
                                    @csrf
                                    @method('PATCH')
                            ...
                            <td class="mailbox-date text-center table-action">
                                ...
                                <form id="delete-item{{$g->id}}" action="{{ route($link.'.destroy',[$link=>$g->id]) }}" method="POST" style="display: none;">
                                    @csrf
                                    @method('delete')
                                </form>
                                <a class="pl-3">
